	...
	<div class="container">
		...
		<table class="table text-center">
			...
			<tbody>
				...
				<c:forEach var="member" items="${membership}"> <%--member는 map형태 --%>
					<tr>
						<td>${member.name}</td>
						<td>${member.phoneNumber}</td>
						<td>
						<c:choose>
							<c:when test="${member.grade == 'VIP'}" >  <%--비교는 대부분 EL안에서 진행하고 ""쓰면 바깥과 혼동되니까 ''을 사용 --%>
								<span class="text-danger">${member.grade}</span>
							</c:when>
							<c:when test="${member.grade eq 'GOLD'}"> <%-- eq , == 둘다 가능 --%>
